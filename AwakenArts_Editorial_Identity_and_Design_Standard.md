...
**Wordmark trace-noise fix (2026-07-14, per Susan)** — the AWAKENARTS wordmark and tagline are raster-traced vector paths (vtracer output, hundreds of small color-stepped shapes approximating the original artwork). The trace left 44 stray "noise" paths embedded inside letter counters and gaps (the hollow triangle of the A, the K's opening, gaps within the tagline's script letters) — pale neutral-to-cool grays (e.g. `#f3f3f3`, `#d2d5da`) that don't belong to either the warm-gold monogram palette or the navy wordmark palette, and don't blend toward the warm cream page background either. Rendered at any real size these read as a mottled, "bad ink job" look across the wordmark. Identified programmatically (any non-monogram path lighter than the navy letterforms but not warm-tinted toward `--cream`) and removed — zero genuine letterform, antialiasing-edge, or monogram paths touched, confirmed by re-rendering and comparing before/after at high zoom. Fixed in `AwakenArts-Logo-Horizontal.svg`, `-Horizontal-NoTagline.svg`, `-Primary.svg`, `-Compact-Horizontal.svg`, and `-Stacked-Colophon.svg` (the five assets that embed the raster-traced wordmark). `-Horizontal-OnNavy.svg` and the single-color `-Navy/-Gold/-Black.svg` variants use live `<text>` or a separately-built simplified outline for the wordmark, not this trace, so were never affected.
