...
- Collection cover (baked into `collection-cover-clean.png`, displayed at 380px, rebuilt 2026-07-14 — see below): monogram renders ≈36px — the tightest placement on the site, worth watching if it ever needs to shrink further (e.g. a smaller cover thumbnail elsewhere).

**Collection cover — clean rebuild (2026-07-14, per Susan).** `public/images/collection/collection-cover.jpg` had accumulated a faint pale rectangular residue band (flagged in the 2026-07-14 closeout QA pass, see Implementation Log) — a hard-edged seam inconsistent with the artwork's watercolor style, left over from an earlier subtitle-removal edit. Rather than patch it, the cover was rebuilt from a fresh flat-cream top zone (feathered into the untouched figure artwork at the exact pixel row the hair begins) carrying only the Compact Horizontal logo and "The Collection" title — nothing else. Saved as a new file, `collection-cover-clean.png`; the original `collection-cover.jpg` is preserved on disk, untouched, and no longer referenced from the homepage.

**Method section leaf ornament (2026-07-14, per Susan, revised same day).** One instance of the approved leaf-pair asset (`brand-assets/awakenarts-leaf-pair.png`, retained brand-library source, unaltered) marks the Method section as a small decorative signature: eyebrow ("The AwakenArts Method") → leaf → opening statement. The website derivative, `public/images/brand/awakenarts-leaf-pair-trimmed.png`, is alpha-trimmed to the source's exact visible-ink bounding box (transparent canvas removed only — no recoloring, cropping, or retracing of the artwork itself). Decorative only (empty alt). Not part of the Approved Logo Family table above — this is the only sanctioned decorative-ornament placement on the site to date; do not extend it to other sections without Susan's direction.
