...
- `AwakenArts_Teaching_Path_Development_Brief.md` carries this principle, the approved foundational homepage question ("Do we use metaphors so routinely that we sometimes stop noticing the images inside our own language?" — not attached to any specific metaphor field), the Familiar-Language Field requirement every Edition must satisfy (Dragon's fire/speech field given in full), the First Participant Crossing pattern, and the fourteen-step Edition Facilitation Map (also working, not frozen).
- **The governing relationship across the system, stated explicitly in `AwakenArts_Path_Standard.md`:** the Path Standard governs what constitutes forward movement; the Teaching Path Development Brief develops and tests the Path; Edition research establishes the substance of each work; Edition facilitation documents determine how that work carries participants across the stones; Presentation, syllabus, and journal serve that established facilitation path. **Do not allow presentation production to determine teaching architecture.**
- **First participant crossings must arise from familiar metaphor, not generic image-response prompts.** Do not write or approve a stage that asks "what does [the Figure] mean to you" as an opening move — the Familiar-Language Field crossing (a known idiom, then the participant's own example, then the noticing-metaphors question) comes first.
- The homepage hero, the foundational metaphor question, and a new "AwakenArts, A Path of Stones" section were implemented on the homepage the same day this correction was issued — see the Implementation Log and this file's own record of exact files changed, made available to Susan for review before any push.

## Goal 7 governance correction — August 20, 2026

**"Laying the Stones," incorporated system-wide, then sharpened by Susan the same day.** A Path stone is not complete because its subject has been researched or because an activity, prompt, or presentation section has been created. Research alone does not constitute a completed stone. **The nine-part Stone Template (`AwakenArts_Teaching_Path_Development_Brief.md`) is a development checklist, not yet the Path.** A stone is laid when we know how a participant gets onto it, what happens while she is there, and why she is prepared to step from it onto the next one — all eight of: ENTRY STATE, FOUNDATION, EDITION EVIDENCE, FACILITATOR MOVEMENT, PARTICIPANT DISCOVERY, WHY IT MATTERS, EXIT STATE, NEXT STONE. From this point forward, "complete a stone" means producing its participant crossing together with its research foundation, not the checklist now and the crossing later. The participant must arrive somewhere different from where she entered.

- `AwakenArts_Path_Standard.md` carries the operational definition (amended twice, 2026-08-20). `AwakenArts_Teaching_Path_Development_Brief.md` reconciles it with the nine-part checklist — the checklist is preparatory; the eight-part sequence is what a stone actually is. Stone One currently satisfies only the checklist and is flagged there, honestly, as not yet a completed stone.
- **Do not generate a workshop stage, slide, or presentation section — for Dragon, Queen Ann, or any Figure — without first stating its Entry State and Exit State.** A CTA or a "continue" link is not, by itself, a reason to continue; the element it closes must have already done its own work (see the Path Standard's amended Test item 3).
- Several existing governing-adjacent documents assume a fixed stage sequence constitutes movement on its own and were marked **SUPERSEDED FOR PATH ARCHITECTURE** in `AwakenArts_Documentation_Map.md`, 2026-08-20, rather than rewritten: `AwakenArts_Workbook/Architecture/AwakenArts_Workshop_Presentation_Standard.docx`, `AwakenArts_Workbook/Book_01_Workshop_System_Blueprint.docx`, and `Dragon_Workshop_Blueprint_New_Paradigm.md`. Read the Map's "Superseded for Path Architecture" table before treating any of the three as current authority on sequence. `/Users/sashe/Desktop/Dragon Pass One.docx` carries the same fixed-sequence assumption but is Susan's own document and is flagged for her review, not marked superseded here.
- A document referenced as "the AwakenArts Participant-Value Standard" could not be located in this repository as of 2026-08-20 — only a completed audit against it, `AwakenArts_Participant_Value_Audit_Findings.docx`, exists here. See the Documentation Map's Reference Materials entry. Do not fabricate or reconstruct this Standard's content; ask Susan where it lives, or whether it should be drafted fresh.
