...
**Documented exception (2026-06-27):** the Encounters index (`/encounters`) intentionally departs from `--band-gap` (6rem) — its "content end → band" gap is now 11rem desktop / 8rem at ≤640px, set directly in `.page`'s `padding-bottom` (`encounters-index.module.css`). Rationale: the page closes on five doorway cards, not a paragraph of prose, so the pause before navigation should read as a deliberate breath — a chance to finish encountering the cards — rather than the standard transition gap used after editorial content elsewhere. In the same pass, the heading group above the cards (eyebrow → title → two-line statement) was tightened slightly (`.intro` margin 4rem → 3rem; internal label/title/statement margins trimmed correspondingly) so the saved space could be spent below the grid instead of above it. Any future page whose last content block is a *terminal visual statement* (cards, a single image, a closing gesture) rather than continuing prose is a reasonable candidate for the same kind of documented override — but it should stay the exception, not the rule; everywhere else, use `--band-gap`.
