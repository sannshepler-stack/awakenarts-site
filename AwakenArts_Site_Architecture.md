...
**"Reading Before Decoration" (added 2026-06-28, per Susan).** A practical restatement of the Readability Primacy Rule above, given as the next checklist to run any time the site's typographic hierarchy is revisited:

> AwakenArts is a publishing platform. Reading comfort and editorial clarity take precedence over minimalist typography. If the reader is expected to read it, it should be comfortably readable.

Applied as four checks: (1) explanatory paragraphs throughout the site stay at or above the `--body-size` floor, never sized down for visual lightness; (2) supporting copy beneath a major quotation reads at the same floor — it is "supporting" in role, not in size; (3) short editorial/credal statements ("Christ is the center.", "Scripture is the authority.", "The artistic work is an invitation to reflect.") carry real typographic presence rather than reading as a small caption, since they are conclusions the reader is meant to register, not fine print; (4) calls to action read confidently — large enough to feel intentional — without tipping into oversized. First applied to the homepage Matthew 13:34 section: `.hero-quote-body` (the explanatory paragraph) bumped to `calc(var(--body-size) + 2px)`, and `.home-foundation-credo` (the "Christ is the center. Scripture is the authority." line) raised from 0.95rem/`--mid` gray to a clearer gold treatment with more weight and breathing room — it had been sitting below the body-copy floor, which is exactly the "too small to feel like it matters" failure this standard exists to catch. `.hero-quote-closing` (the "Read the Foundation of AwakenArts →" CTA) was reviewed and left as-is — already sized to read as a confident, deliberate CTA without the section's quotation or credo overtaking it.
