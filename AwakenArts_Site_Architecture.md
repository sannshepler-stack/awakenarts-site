...
**"Pause" tier — `--band-gap-pause` / `--band-gap-pause-md` / `--band-gap-pause-sm` (added 2026-06-27, globals.css :root):** a second, longer tier of the same rhythm, for the rare page whose last content block is a terminal visual statement (a card grid, a single image, a closing gesture) rather than continuing prose, and so earns a longer breath before the Wayfinding Band than the standard transition. Fixed at 1.5x `--band-gap` at every breakpoint (9rem / 7.5rem / 6rem vs. the standard 6rem / 5rem / 4rem) so it stays a defined multiple of the universal rhythm rather than a one-off page-specific number — substantial, but never out of scheme with the rest of the page. This replaces the original 2026-06-27 fix's hardcoded 11rem/8rem values, which read slightly too generous once the structural bug below was corrected and the gap actually became visible; 1.5x reads as a deliberate pause without overshooting it.

Use `--band-gap` by default. Reach for `--band-gap-pause` only when a page's last section is a terminal visual statement, not prose — currently just the Encounters index (`/encounters`, `.page` in `encounters-index.module.css`). In that same pass, the heading group above the cards (eyebrow → title → two-line statement) was tightened slightly (`.intro` margin 4rem → 3rem; internal label/title/statement margins trimmed correspondingly) so the saved space could be spent below the grid instead of above it. Any future page that qualifies should reference these same three tokens rather than hand-picking its own value, so the "pause" stays one consistent, named idea site-wide instead of drifting into per-page guesswork.
