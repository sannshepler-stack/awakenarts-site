...
## 2026-07-12 — Dragon Edition: production synchronization pass

Following the previous entry, Susan issued a formal Production Master Directive establishing a document-authority order (latest approved decisions → master manuscript → production layout files → historical review documents → archived drafts) and asked for a synchronization checklist across the manuscript, the production layout, and the published PDF before any further edits — see `Dragon_Production_Synchronization_Checklist.md` for the full item-by-item comparison.

**Two real errors caught by the audit, corrected here.** (1) The prior pass had applied the new atmospheric art (`dragon-background.png`) as a CSS background behind the *poem text* section. Cross-checking against the approved `Dragon_Figure_Edition_11_Series_Prototype.pdf` (2026-07-07) showed its actual "page 4" is a pure full-bleed image with no text — the art belonged on the website's `image` section (replacing the old full-dominance dragon illustration), not behind `word`. Corrected: new responsive JPGs regenerated for the `image` section from the source PNG; the erroneous background removed from `word`. (2) The website had `image` before `encounter` (Begin); the approved layout has Begin before the image page. Reordered to match.

**Manuscript.** Per Susan's follow-up directive ("the manuscript is the master, the PDF is a build artifact"), `Dragon_Figure_Edition_11_Good_Copy.docx` now opens with a dated "Production Synchronization" note containing today's governing text for every revised page (Recognition, The Larger Story, Reflection, Journal, Message Delivered, Living the Message, About This Edition), explicit about which older wording elsewhere in the same file it supersedes. Nothing was deleted or rewritten — the existing Good Copy content, revision logs included, is preserved unedited below the note. Edited via unzip → XML paragraph insertion → rezip; validated with the docx skill's XSD validator (paragraphs 87→161, all checks passed) and rendered to PDF via LibreOffice for a visual check before being placed back in the Workbook.

**Published PDF.** `public/files/editions/Dragon_Figure_Edition.pdf` was a stale 9-page file from June 21 — older than even the original Good Copy master, let alone today's revisions. Rebuilt as an 11-page PDF (WeasyPrint, HTML/CSS source) using today's approved text and the real production art assets, page-for-page matching the website Reader. One substitution: Cormorant Garamond isn't fetchable from this sandbox (no network path to `fonts.gstatic.com`), so the PDF uses Liberation Serif instead — flagged explicitly in the checklist as a known limitation, not silently absorbed. A copy of the new PDF was also placed in the Workbook (`Dragon_Figure_Edition_11_Current_Production.pdf`) alongside the Series Prototype, for reference.

**Checklist.** `Dragon_Production_Synchronization_Checklist.md` records what's now confirmed in sync (Living the Message, The Larger Story, Recognition, Reflection, Journal, Message Delivered, page order, Page 4 art, Facilitator Notes' intentional absence) and three open items that need Susan's decision rather than a unilateral call: whether a standalone "THE DRAGON" title page belongs back in the sequence, whether the Reader should carry the print layout's per-page footer, and whether the poem should ultimately be composited directly onto the Image page's own art rather than kept as a separate page.

**Verification.** `tsc --noEmit` clean. New `image` section JPGs and the regenerated PDF's 11 pages were each rendered and visually checked (WeasyPrint output via `pdftoppm`) against the production doc's reference art before being committed. Manuscript docx validated (XSD + structural check) and rendered to PDF for a visual read-through.

**Implementation status.** Complete, with three open items awaiting Susan's decision (recorded in the checklist, not treated as resolved).

Susan also adopted a standing workflow rule, effective from this point forward: Workbook folders (`AwakenArts_Workbook/...`) are where writing, editing, and production happen; `public/files/editions/` holds only the final PDFs the website publishes. On every future edition approval — update the master manuscript first, generate a new PDF from it, replace the corresponding file in `public/files/editions/`, then commit both together.

## 2026-07-12 — Dragon Edition: Revision Directive, full page-by-page art pass (round 2, same day)

Later the same day, Susan issued a detailed eleven-page "Dragon Revision Directive," reviewing the synchronization pass above page by page. No reader-facing text changed in this round — every instruction was art, layout, or asset-level. Full item-by-item detail is in `Dragon_Production_Synchronization_Checklist.md`'s "round 2" table and in the manuscript's own new "ADDENDUM — DRAGON REVISION DIRECTIVE" note (added per the adopted manuscript-first workflow rule, immediately above where the round-1 "Production Synchronization" note left off).

**The Image (Figure) — second reversal, same day.** Round 1's audit had (correctly, at the time) moved the atmospheric "Dragon Fight" field onto the `image` section. The Directive reverses this: the Gallery's own painted dragon figure (`dragon-figure.jpg`, teal/gold watercolor) belongs on the Figure page — confirmed against the master manuscript's own Page 3 production note, which independently describes "the framed watercolor dragon figure on a cream mat." The atmospheric field moves to the Poem page instead, as its background.

**The Poem — rebuilt, not just recolored.** The previous rounds' plain-HTML poem text is removed entirely. Susan pointed to the real artwork's actual location and pasted a reference image; the file that visually matched exactly (title, colors, closing lines, transparent background) was `AwakenArts_Workbook/02_Dragon/Images/01_Master_Poem/Dragon_Poem_Master.png`, not the superficially similar `_masters/dragon-text.png` (which had a broken transparency mask and didn't match). `EditionSection` gains an `overlayImage` field (`{ src, alt }`) for this exact case — a background plus one piece of real artwork on top, no HTML text — and `EditionReaderSection.tsx` renders it before the generic `image`/`text` branches.

**Encounter — restored.** Background art (storm clouds parting to a volcanic sunset) added back; this page had drifted to text-only against Susan's explicit "do not leave this page text-only" instruction. Caught in review: the restored art is very dark, so `.reader-section--encounter.reader-section--has-bg .reader-text` was given light/cream text with a soft shadow — the section's default deep-navy text would have been unreadable against it.

**Larger Story / Reflection — atmosphere added.** Larger Story gets a full-page background (landscape, sun and moon, faded 72% toward the page cream) behind its unchanged, already-approved text. Reflection gets a banner image above its unchanged text, the same swirl-motif art family and `<img>`-banner treatment already used for Recognition (`useBanner` in `EditionReaderSection.tsx` now covers both ids).

**Journal — art and lines separated.** The full-page `journal-bkg.png` (ruled lines baked in under the whole page) is replaced with a new asset, `journal-arch.png`: the same arch/foliage illustration, tightly cropped to just its own corner, with its baked-in ruled lines erased (a vertical box blur along the y-axis, kernel 15–21px, erases 1–2px horizontal rules while preserving the coarser watercolor brushwork) and its top/left edges alpha-feathered so it composites seamlessly as a bottom-right corner accent rather than a full-bleed cover. Real ruled lines (plain CSS `border-bottom` divs) are rendered beneath the intro text only, in the open writing space below it — `reader-journal__lines`/`reader-journal__line`.

**Colophon — bottom band, not full cover.** Rather than lean on CSS `mask-image` (broadly supported in real browsers, but unverifiable in this sandbox and unsupported by WeasyPrint, which would have made the print PDF impossible to visually check), the fade is pre-baked as its own asset: `colophon-band.png` is the original `colophon-bg.jpg`'s bottom ~52%, with its own top edge alpha-feathered to transparent via PIL/numpy, so it renders identically — and was actually visually verified — in the website, in a WeasyPrint mockup, and in the regenerated print PDF. Rendered as a real `<img>` positioned to the bottom of the section, not a CSS background.

**Living the Message.** Background re-faded further (55% toward page cream, up from round 1's unfaded version) so the arch reads as atmosphere behind the text rather than dominating the page, per the Directive's explicit instruction.

**Verification.** `tsc --noEmit` clean throughout. `next build` still cannot run in this sandbox (the linux/arm64 native SWC binary installs as an empty stub — a pre-existing, disclosed limitation, not new to this round). In its place: every new/changed image asset was visually inspected directly; the four structurally new treatments (Poem overlay, Journal corner + lines, Reflection banner, Colophon band) were built as a standalone HTML mockup using the exact CSS rules added to `globals.css` and rendered through WeasyPrint (already in use for the print PDF) to PNG for a real visual check, catching the Encounter text-contrast issue before it shipped. The production PDF (`dragon-pdf/build.py`) was updated to match every page treatment 1:1 with the website and regenerated; `pdfplumber` confirmed 11 pages, correct image counts per page (2 on the Poem page: background + overlay), and all expected text. Manuscript docx validated (XSD + structural check, 161→176 paragraphs) and rendered to PDF for a visual read-through of the new addendum note.

**Implementation status.** Complete. `public/files/editions/Dragon_Figure_Edition.pdf` and the Workbook reference copy are both replaced with the new 11-page build; the manuscript's addendum note and this checklist are the record of what changed and why. The two items still open from round 1 (standalone title page, per-page web footers) remain open — this round didn't touch either.

## 2026-07-14 — Homepage & Brand Foundation: final closeout QA pass

Susan approved the homepage and responsive foundation and asked for a final quality-assurance pass before moving on to the Dragon Companion Edition prototype: no further design/identity/spacing/typography/color/content revisions except to correct a verified defect.

**Visual QA, five breakpoints (1728 / 1440 / 1024 / 768 / 390px).** Rendered via the session's Next-dev-server + WeasyPrint pipeline (real server-rendered CSS, no hand mockups); breakpoint-specific states below 1080px verified with a forced-state stylesheet override, since WeasyPrint doesn't evaluate `max-width` media queries against a rendered page size. Confirmed at every width: no horizontal scroll, nav logo and hero content share the same left edge, desktop hero/nav unchanged above 1080px, compact/stacked treatment below it, hero artwork never cropped horizontally, the Matthew quotation's fade-to-quote pause reads correctly at actual size, footer shows all four social links on desktop.

**Queen Ann poem.** Confirmed via `git diff` across every commit in this work session that no change touched `.poems-showcase-ann*` (the shaped poem's frame/text/image classes) or its underlying image asset — the poem's shape, line breaks, and proportions are provably untouched, not just visually similar.

**Contrast and buttons.** Re-ran the WCAG contrast calculation for the current `--gold` (#7E5F12) against all three light backgrounds — 5.56:1 / 4.97:1 / 5.09:1, all still pass AA-normal. Grepped for any remaining hardcoded `#8B6914` outside of comments — none found. Audited hover states site-wide: all homepage buttons/links are color-shift only except `.home-coll-cta` ("Explore the Collection"), which carries a pre-existing `transform: translateY(-1px)` hover lift that predates this session — flagged for Susan's decision rather than silently changed.

**Brand asset audit.** No references to `public/archive/deprecated/` anywhere in `src/`. All four approved outlined vector masters present in `brand-assets/vector-masters/` and untouched this session (confirmed via `git log` on the relevant paths). No vector master was replaced by a PNG. Favicon, hero, footer, and nav each confirmed on their correct approved asset. Two placements remain on older/unresolved assets, both already documented as open decisions before this pass (not new findings): the reader colophon still uses `AwakenArts-Logo-Primary.svg` rather than the purpose-built `AwakenArts-Logo-Stacked-Colophon.svg`, and no decorative emblem watermark has been implemented anywhere. `brand-assets/README.md`'s Pending Decisions section updated to reflect what this session actually resolved (footer/studio/quotes on-navy compact logo, nav logo wired) versus what's still open.

**Collection cover asset check.** `public/images/collection/collection-cover.jpg` has a faint pale rectangular band on the right side, roughly 820–1122px × 320–480px in the 1122×1402 source, overlapping a fainter organic sketch-line cloud/rock shape. The organic linework reads as intentional (matches the piece's watercolor-sketch style elsewhere). The rectangular band does not — hard straight edges, flat tone, inconsistent with the hand-drawn style around it — and its position is consistent with a residual seam from the earlier subtitle-removal edit on this same cover (see Implementation Log/task history, "Collection cover: swap to approved Compact Horizontal lockup" and the earlier subtitle-removal pass). Reported per instruction; not removed without confirmation.

**Documentation.** `AwakenArts_Editorial_Identity_and_Design_Standard.md` updated: full Atmosphere System palette (was still showing the pre-contrast-fix `--gold` value and missing all nine roles), a new Responsive Breakpoint Standard (1080px, nav+hero collapse together, never split), a Button/Link Interaction Standard (color-shift only, with the one known exception flagged), and a Shaped/Concrete Poem Preservation Rule (never reflowed/rewritten/reformatted/reconstructed). Logo-placement table's Navigation row confirmed now actually implemented, not just planned.

**Technical closeout.** `npx tsc --noEmit` run clean (see below); `next build`'s known sandbox limitation (missing native ARM64 SWC binary, pre-existing and disclosed, not new to this pass) is worked around by using `next dev` to render every proof in this pass against real, server-compiled CSS/HTML — the same source files a production build would use. The commit closing this pass is recorded in the final closeout report delivered alongside this entry.

**Implementation status.** QA complete. Two flagged, non-blocking items await Susan's decision (the `.home-coll-cta` hover transform; the Collection cover's pale rectangular band) — neither was altered. No design/identity/spacing/typography/color/content changes were made in this pass beyond the documentation updates described above. Per Susan's direction, no deployment was performed. Next production task: Dragon Companion Edition prototype (Poppy is not the Edition template).

## 2026-07-14 — End-of-day closeout: Collection cover rebuild, Method leaf placement, Dragon handoff

Two approved changes closed out same day as the QA pass above, plus the standing pale-rectangular-band item from that pass is now resolved as a side effect of the first change.

**Collection cover — clean rebuild (commit `7aa8bb6`).** Per Susan's "Collection Book Cover — Clean Rebuild" directive. The pale rectangular residue band flagged in the closeout QA pass above is gone: the top zone of the cover was rebuilt as a fresh flat-cream field (background tone sampled from the source itself, feathered into the original artwork starting at the first row of hair pixels — measured pixel-by-pixel so the figure is never touched, cropped, or regenerated). The new top zone carries only the two approved elements: the Compact Horizontal logo (no tagline, small, centered, generous clear space) and "The Collection" title in Cormorant Garamond SemiBold navy. Saved as a new file, `public/images/collection/collection-cover-clean.png`; the source `collection-cover.jpg` is untouched and still on disk. Homepage Collection section now points at the clean PNG. Approved by Susan after actual-size cover and homepage-placement proofs.

**Method leaf — size and placement revision (commit `b129670`).** Per Susan's follow-up directive, the leaf-pair ornament moved from above the eyebrow to directly below it: eyebrow ("The AwakenArts Method") → leaf signature → opening statement. A new tightly-trimmed derivative, `public/images/brand/awakenarts-leaf-pair-trimmed.png`, was cut to the exact alpha-channel bounding box of `brand-assets/awakenarts-leaf-pair.png` (removing only transparent canvas, no visible leaf artwork altered); the untrimmed source in `brand-assets/` is unchanged. Sized 36px desktop / 30px mobile, natural aspect ratio preserved, decorative empty alt. Approved by Susan after actual-size desktop and mobile proofs.

**Retained brand-library assets.** `brand-assets/awakenarts-leaf-pair.png` remains the retained source master (untouched); `brand-assets/awakenarts-a-watermark.png` and `brand-assets/awakenarts-botanical-sprig.png` remain retained, still unused on the site — both already logged as open/pending decisions, unchanged status.

**Color-only button behavior.** Re-confirmed unchanged from the QA pass above: all homepage buttons/links are color-shift only on hover, with the one pre-existing, already-flagged exception (`.home-coll-cta`'s `translateY(-1px)`), not touched.

**Documentation.** This entry, plus `AwakenArts_Editorial_Identity_and_Design_Standard.md`'s Collection Cover and Method Section notes and `brand-assets/README.md`'s Pending Decisions section, updated to reflect both changes above (see those files for specifics).
